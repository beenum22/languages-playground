    ...
    fn eq(&self, other: &Self) -> bool {
        ...
        for i in 0..self.count {
            if self.terms[i].coefficient != other.terms[i].coefficient
                || self.terms[i].exponent != other.terms[i].exponent
            {
                return false;
            }
        }
...
impl Polynomial {
    pub fn new(count: usize) -> Self {
        ...
        Polynomial { count, terms }
    }

    pub fn set_term(&mut self, coefficient: i32, exponent: u32) {
        if self.terms.get_len() == self.terms.get_size() {
            self.terms
                .resize(self.terms.get_len() + 1)
                .expect("Failed to increase the Polynomial terms count!");
            self.count += 1
        }
    ...
    pub fn add(&mut self, other: Self) {
        ...
        while i < self.count && j < other.count {
            if self.terms[i].exponent == other.terms[j].exponent {
                res.set_term(
                    self.terms[i].coefficient + other.terms[j].coefficient,
                    self.terms[i].exponent,
                );
                i += 1;
                j += 1;
        ...
        self.terms
            .resize(self.count)
            .expect("Failed to resize the Polynomial terms array!");
    }
}
...
mod polynomial {
    ...
    fn test_new() {
        ...
        assert_eq!(
            poly.terms.get_len(),
            0,
            "Invalid Polynomial terms array length. It should be 0!"
        );
        assert_eq!(
            poly.terms.get_size(),
            count,
            "Invalid Polynomial terms array size. It should be set to the terms count!"
        );
    }
    ...
    fn test_set_term() {
        ...
        assert_eq!(
            poly.terms.get_len(),
            2,
            "Invalid Polynomial terms array length. It should be 2!"
        );
        poly.set_term(2, 0);
        assert_eq!(
            poly.terms.get_len(),
            3,
            "Failed to extend the Polynomial length!"
        );
    ...
    fn test_evaluate() {
        ...
        assert_eq!(
            poly.evaluate(x),
            23,
            "Invalid Polynomial evaluation. Outcome should be 2!"
        );
    }
    ...
    fn test_add() {
        ...
        assert_eq!(
            p1.count, 4,
            "Invalid Polynomial count after addition. Outcome should be 4!"
        );
        assert_eq!(
            p1.terms.get_len(),
            4,
            "Invalid Polynomial terms array length after addition. Outcome should be 4!"
        );
        assert_eq!(
            p1.terms.get_size(),
            4,
            "Invalid Polynomial terms array size after addition. Outcome should be 4!"
        );
